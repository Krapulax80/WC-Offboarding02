cls
. \\BNWINFRATS01.westcoast.co.uk\c$\Scripts\AD\OffBoarding\Functions\Create-Credential.ps1
. \\BNWINFRATS01.westcoast.co.uk\c$\Scripts\AD\OffBoarding\Functions\Connect-OnlineExchange.ps1
$OutputFolder = "\\BNWINFRATS01.westcoast.co.uk\c$\Scripts\AD\OffBoarding\Output"
...
function Process-OffBoarding02 {
    ...
    [Parameter(Mandatory=$false)] [switch]$First5,
    [Parameter(Mandatory=$false)] [string]$OutputFolder
  )

  #Configuration definition
     $infraserver = "BNWINFRATS01.westcoast.co.uk"

      if ($Westcoast.IsPresent){
        # Credentials for WC
        Create-Credential -WestCoast -AD -CredFolder "\\$infraserver\c$\Scripts\AD\OffBoarding\Credentials\"
        Create-Credential -WestCoast -AAD -CredFolder "\\$infraserver\c$\Scripts\AD\OffBoarding\Credentials\"
        # Config file
        $config = Import-Csv "\\$infraserver\c$\Scripts\AD\OffBoarding\config\westcoast.csv"
      }
      elseif ($XMA.IsPresent){
        # Credential for XMA
        Create-Credential -XMA -AD -CredFolder "\\$infraserver\c$\Scripts\AD\OffBoarding\Credentials\"
        Create-Credential -XMA -AAD -CredFolder "\\$infraserver\c$\Scripts\AD\OffBoarding\Credentials\"
        # Config file
    ...
    if ($First5.IsPresent) {
    $number = 5
    Write-Host "WARNING (!) You are in 'First5' mode. Only the first $number results are returned. For real work ommit the '-First5' switch !" -ForegroundColor Yellow
    $InactiveLeavers  = Get-ADUser -Credential $AD_Credential -Server $DC -Filter * -SearchBase $searchBase -Properties *  |Where-Object {($_.LastLogonDate -EQ $null) -or ($_.LastLogonDate -lt $inactiveDate) -and ($_.Name -notlike 'svc.*') } | Select-Object Name,EmailAddress,LastLogonDate,whenCreated,UserPrincipalName,SAMAccountName | Sort-Object whenCreated | Select-Object -First $number
    }
    else {
    ...
    $Obj = $null ; $Obj = New-Object -TypeName PSObject
        ...
        #Actual mailbox name
        $MailboxAddress=$mailboxfound = $null
        ...
        if (Get-Mailbox $MailboxAddress -ErrorAction SilentlyContinue) {
            ...
            #Report in array
            $details = Get-Mailbox $MailboxAddress | select Name,*type*
    ...
    # ACTION - SUMMARY
    if ($Summary.IsPresent) # First5 MODE - display calculation for check
    {
    ...
    Write-Host "The number of leavers without mailboxes: $($UsersWithoutMailbox.count)" -ForegroundColor Magenta ; $UsersWithoutMailbox | Export-Csv -Path ($OutputFolder + "\" + $today + "_" + $DomainNetBIOS +  "_leavers_WITHOUT_mailbox.csv" ) -Force
  ...
  #Process-OffBoarding02 -WestCoast -Live -First5 -Summary -OutputFolder $OutputFolder

  Process-OffBoarding02 -XMA -Dryrun -Summary -OutputFolder $OutputFolder
  Process-OffBoarding02 -WestCoast -Dryrun -Summary -OutputFolder $OutputFolder
  
# SIG # Begin signature block
# MIIOWAYJKoZIhvcNAQcCoIIOSTCCDkUCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUr8FjjkfjDAqo5tAiiD1KUJyW
# ZFagggueMIIEnjCCA4agAwIBAgITTwAAAAb2JFytK6ojaAABAAAABjANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQU868OklOEKrG52pPwjyoBG34mH0wwDQYJKoZI
# hvcNAQEBBQAEggEAMoPe5yFgX5G2d9atXqJ/cB/dFG1WcZajuiXVKSyU6Tn03FhT
# Zi4qyPJ620CbvB1l6/pXSb8SbTfoEXJ1gJj/PRrPbr44yiHalNF796+j1tcFoljM
# VO0QOA/rFGSEFMsU/yB3/N5YANCQfF/kzHk1GpulNHpJl7INFWK2FWTc0LcnwovS
# w9DY6rTKniaZPuG0wlGVZteu3BoqrwwMWBeokkKF9LxeW1Nnu8XCYKraEGZZJ9H5
# sYDGFJTKDmiYRGM+suXLrBymKD2XJNmOuUGJ+NmflE7fHx1Fz2H8US61WTlekKZv
# 5Qj/U2Y9jw1DVjTZ/xxY6pDmqtASuiL/qOdP2g==
